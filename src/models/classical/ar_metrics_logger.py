...
entry = pd.DataFrame([{"Model": "AR", "RMSE": round(rmse, 4), "MAE": round(mae, 4), "MAPE": round(mape, 2)}])
if os.path.exists(metrics_file):
    existing = pd.read_csv(metrics_file)
    existing = existing[existing["Model"] != "AR"]
...
plt.plot(df["forecast"], label="AR Forecast", linestyle="--")
plt.title("AR(2) Actual vs Forecast")
plt.legend()
plt.savefig(os.path.join(figs_path, "ar_actual_vs_forecast.png"))
plt.close()
